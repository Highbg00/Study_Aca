...
SELECT  employee_id 사번, last_name 성, department_id 부서코드, job_id 업무코드, hire_date 입사일자, 
        manager_id,
        NVL(manager_id,0) "없으면0(NVL)",
        NVL(TO_CHAR(manager_id),'없음') "없으면없음(NVL)",
        NVL2(manager_id,'있음','없음') "매니저있음없음(NVL2)",
        NVL2(manager_id, TO_CHAR(manager_id), '없음') "manager_id 없음(NVL2)",
        NVL2(manager_id,'O','X') "있으면O없으면X(NVL2)",
        NVL2(manager_id, 'Manager', 'No Manager') "Manager No Manager(NVL2)"
FROM    employees;
