$(document).ready(function() {
	...
	var imgIdx = 1;	//처음에 3초 보여주고 setInterval로 3초 후에 2번째 보여줌
	...
	//캔버스에 올려놨을때 정지 후 캔버스에서 나왔을때 다시 실행
	$("animation_canvas").on({
		"mouseenter" : function() {
	...
	//control_button의 클릭 리스너 및 data-index 할당
	$(".control_button").each(function(index) {
		$(this).attr("data-index",index);
	}).click(function() {
		var index = $(this).attr("data-index");
		imgIdx = index;	//반드시필요, 내가 클릭한 index번호를 3초마다 돌리는 imgIdx 변수값으로 전달
										//랜덤하게 클릭했을때 다음 그림으로 이동
		//그림 및 덱스트 불러오기 : moveSlider()함수
		moveSlider(index);
	...
	//초기 0번째 인덱스 텍스트 가져오기 및 0번째 컨트롤 버튼 설정
